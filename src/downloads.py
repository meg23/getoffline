...
def skip_active_live_videos(info_dict, *, incomplete):
    """Skip currently active live streams for YouTube entries."""

    live_status = (info_dict.get("live_status") or "").lower()
    if info_dict.get("is_live") or live_status == "is_live":
        return "Skipping active livestream."
    return None
...
for entry in config.get("youtube", []):
    try:
        ...
        ydl_opts = {
            ...
            "match_filter": skip_active_live_videos,
            "ignoreerrors": True,
        }

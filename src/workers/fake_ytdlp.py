...
class YoutubeDL:
    ...
    def extract_info(self, url: str, download: bool = True) -> dict[str, Any]:
        # The combined Compose integration test uses this fake for both the
        # YouTube and podcast downloader services. Podcast episodes have
        # different URLs but do not have a YouTube id, so give each URL a
        # stable synthetic id; otherwise concurrent podcast jobs overwrite
        # the same source file before the FFmpeg worker consumes it.
        video_id = _synthetic_id(url)
        info = {
            "id": video_id,
            "title": _TITLE,
...
def _synthetic_id(url: str) -> str:
    if "youtube.com/" in url or "youtu.be/" in url:
        return _VIDEO_ID
    return hashlib.sha1(url.encode("utf-8"), usedforsecurity=False).hexdigest()[:11]

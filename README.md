...
A Docker Compose deployment is available for the Django frontend, nginx proxy, RabbitMQ, MySQL, and all worker types. MySQL data is persisted in the named `mysql-data` volume, RabbitMQ data is persisted in `rabbitmq-data`, and downloaded media is bind-mounted from `GETOFFLINE_DOWNLOADS_DIR` (default `./downloads`). Set `GETOFFLINE_DJANGO_SECRET_KEY` and optionally override `GETOFFLINE_DB_NAME`, `GETOFFLINE_DB_USER`, `GETOFFLINE_DB_PASSWORD`, and `GETOFFLINE_DB_ROOT_PASSWORD`, then run `docker compose up --build -d`. The frontend is served through nginx on `http://localhost:8080`, while RabbitMQ management is exposed on `http://localhost:15672`. Run schema setup with `docker compose --profile tools run --rm migrate` before starting the stack or after pulling model changes.

...
def _configure_youtube_download_filters(
    ...
    try:
        settings = ProfileDownloadSettings.objects.filter(profile_id=job.profile_id).first()
        if settings and settings.youtube_cookie_text:
            cookie_text = str(settings.youtube_cookie_text).strip()
            if cookie_text:
                # Write cookies to temporary file for yt-dlp
                # Add Netscape format header if not already present

...
class SummarizationTests(unittest.TestCase):
    ...
    def test_llama_cpp_model_ref_allows_filename_override(self):
        with patch.dict(
            os.environ, {LLAMA_CPP_FILENAME_ENV: "custom.gguf"}, clear=False
        ):
    ...
    def test_internal_summary_uses_loaded_llama_model(self):
        ...
        fake_llm.create_chat_completion.return_value = {
            "choices": [
                {"message": {"content": json.dumps({"summary": "Internal summary."})}}
            ]
        }
        with patch(
            "workers.summarization._load_llama_cpp_model", return_value=fake_llm
        ):
            summary = _internal_llama_summary(
                "Some transcript text.", model_name="qwen2.5:0.5b"
            )
    ...
    def test_summarize_segments_uses_internal_backend_by_default(self):
        with patch.dict(os.environ, {SUMMARY_BACKEND_ENV: ""}, clear=False):
            with patch(
                "workers.summarization._internal_llama_summary",
                return_value="Internal summary.",
            ) as internal_summary:
    ...
    def test_ollama_summary_posts_to_environment_url_when_backend_is_ollama(self):
        ...
        class FakeResponse:
            ...
            def read(self):
                return json.dumps(
                    {"response": json.dumps({"summary": "Short summary."})}
                ).encode("utf-8")
        ...
        with patch.dict(
            os.environ,
            {OLLAMA_URL_ENV: "http://ollama:11434/api/generate"},
            clear=False,
        ):
            with patch(
                "workers.summarization.request.urlopen", side_effect=fake_urlopen
            ):

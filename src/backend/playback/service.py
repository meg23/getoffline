...
def apply_update(item: Download, update: PlaybackUpdate, *, now=None) -> PlaybackState:
    ...
    log.info("playback progress download_id=%s position=%.3f reason=%s completed=%s delta=%.3f", item.id, update.position, update.reason or "unknown", update.completed, update.listened_delta)
    item.save(update_fields=update_fields)
    return PlaybackState(item.id, float(item.last_position_seconds or 0.0), bool(item.played), float(item.total_listened_seconds or 0.0))

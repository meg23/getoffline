...
To transfer over Wi-Fi, pair the device with `adb` first, then switch **ADB connection** to **Wi-Fi (connect to paired device)** in settings and enter the device address, such as `192.168.1.50:5555`. Use the device's Wi-Fi debugging connection address/port, not the temporary pairing-code port. GetOffline starts the adb server, runs `adb connect <address>` before each transfer/delete job, and then uses that Wi-Fi serial for normal `adb push`, shell, and media-scan commands. If you omit a port, GetOffline defaults to `:5555`.
